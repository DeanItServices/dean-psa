...
- Caddy overwriting `X-Forwarded-For` converts the existing rate limiter from decorative to genuinely effective without touching the proxy/middleware file — the fix the code comment says can only come from infrastructure.
  ...
  — **CORRECTED** (2026-09-16, `08-03-SUMMARY.md` "The X-Forwarded-For finding — and a correction"): Caddy does not *overwrite* `X-Forwarded-For`; by default it **drops** the client-supplied value, and it does so **conditionally** — the moment a `trusted_proxies` range covers the peer it forwards the full incoming chain with the untrusted left-most entry first, which is exactly the entry `getClientIp()` reads. Verified live against `caddy:alpine` v2.11.4. The conclusion above still holds, but the guarantee comes from the `header_up X-Forwarded-For {remote_host}` directive the shipped `Caddyfile` carries, not from Caddy's default. The same imprecision appears in the MVP checklist below ("Caddy owns 80/443 and overwrites `X-Forwarded-For`") and is corrected by the same finding.
...
- [ ] Remove the host-published `db` port; replace `postgres:postgres` with generated secrets sourced from `.env` in **all three** places (`db.POSTGRES_PASSWORD`, `app.DATABASE_URL`, `email-poller.DATABASE_URL`)
  ...
  — **SUPERSEDED** (2026-09-16, `08-03-SUMMARY.md` "db port — approved deviation, restated"): the `db` port is **loopback-bound**, not removed — `- "127.0.0.1:${DB_PORT:-5432}:5432"`, rendering as `127.0.0.1:5432->5432`. Literal removal strands `db:migrate:deploy`, `db:seed`, `bootstrap:admin` and the E2E suite: the runner image is `node:20.20-alpine` with no `bash` or `psql`, and `bootstrap:admin` refuses non-interactive exec. Loopback closes the off-host exposure while keeping host tooling working. **User-approved deviation.** The credential half of this item shipped as written.
