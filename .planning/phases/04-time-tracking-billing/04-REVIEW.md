...
- OAuth access/refresh tokens are stored in plaintext `@db.Text` columns with no application-level encryption at rest — accepted as reasonable for this project's self-hosted, single-org, <25-user threat model, but not yet documented as an accepted risk anywhere in code/docs.
